...
void Converter::convert() {
    ...
    snprintf(buffer, 11, "%02d.%05.2f%c", fullat_deg, fullat_min, fullatdir);
    out << buffer << ", ";
    for(int i = 0; i < 11; i++) {
        buffer[i] = '\000';
    }
    snprintf(buffer, 11, "%03d.%05.2f%c", fullon_deg, fullon_min, fullondir);
    out << buffer << std::endl;
    for(int i = 0; i < 11; i++) {
        buffer[i] = '\000';
    }
    snprintf(buffer, 11, "%02d.%05.2f%c", flrlat_deg, flrlat_min, flrlatdir);
    out << buffer << ", ";
    for(int i = 0; i < 11; i++) {
        buffer[i] = '\000';
    }
    snprintf(buffer, 11, "%03d.%05.2f%c", flrlon_deg, flrlon_min, flrlondir);
    out << buffer << std::endl;
    out << mapname << std::endl;
    out << "Generated by OziExplorer to UI-View Converter (C) 2015 by Phitherek_ SO9PH" << std::endl;
    out.close();
}
